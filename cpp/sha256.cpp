...
// initialize h values, which are the first 32 bits of the fractional
// parts of the square roots of the first 8 primes (2-19)
const uint64_t hValues[8] = [0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
				 			 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19];

// initialize array of round constants, which are the first 32 bits of
// the fractional parts of the cube roots of the first 64 primes (2-311)
const uint64_t kValues[64] = [0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
...
uint32_t* SHA256(std::vector<char>) {
	...
	const unsigned long long numBlocks = padded.sizeof() / 512;

	int messageChunk[numBlocks];

	// separate padded message into 512-bit chunks
	for(int i = 0; i < numBlocks; i++) {
		messageChunk[i] = (padded >> (512 * (numBlocks - i - 1))) & 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
	}
	...
	for(int i = 0; i < 8; i++) {
		finalHash = finalHash.append(hValues[i]);
	}
...
uint32_t* pad(std::vector<char> message) {
    ...
    if(remainder > 448) {
    	// there is not enough space for the 64 bit size
    	...
    	// exactly a multiple of 512 bits -> append 0b1000_0000
        message[lastIndex] = 0x80;
        lastIndex++;
